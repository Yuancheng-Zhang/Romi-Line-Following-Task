...
volatile bool oldRight_A;  // used by encoder to remember prior state of A
volatile bool oldRight_B;  // used by encoder to remember prior state of B

volatile long count_left; // used by encoder to count the rotation
volatile bool oldLeft_A;  // used by encoder to remember prior state of A
volatile bool oldLeft_B;  // used by encoder to remember prior state of B


// Global volatile timestamps to determine
// speed from within the encoder ISR.
/*volatile unsigned long left_interval;
volatile unsigned long left_prev_time;
volatile int left_direction;
volatile unsigned long right_interval;
volatile unsigned long right_prev_time;
volatile int right_direction;*/
...
ISR( INT6_vect ) {
  ...
  boolean newRight_B = digitalRead( E1_B_PIN );
  boolean newRight_A = digitalRead( E1_A_PIN );

  // Some clever electronics combines the
  // signals and this XOR restores the
  // true value.
  newRight_A ^= newRight_B;
  ...
  byte state = 0;
  state = state | ( newRight_A  << 3 );
  state = state | ( newRight_B  << 2 );
  state = state | ( oldRight_A  << 1 );
  state = state | ( oldRight_B  << 0 );
  ...
  switch ( state ) 
  {
    case 1:  count_right--; break;  // clockwise?
    case 2:  count_right++; break;  // anti-clockwise?
    case 4:  count_right++; break;  // anti-clockwise?
    case 7:  count_right--; break;  // clockwise?
    case 8:  count_right--; break;  // clockwise?
    case 11: count_right++; break;  // anti-clockwise?
    case 13: count_right++; break;  // anti-clockwise?
    case 14: count_right--; break;  // clockwise?
    default:                break;  // No movement.
  }

  // Save current state as old state for next call.
  oldRight_A = newRight_A;
  oldRight_B = newRight_B;
...
ISR( PCINT0_vect ) 
{
  ...
  boolean newLeft_B = PINE & (1<<PINE2);
  //boolean newLeft_B = PINE & B00000100;  // Does same as above.

  // Standard read fro the other pin.
  boolean newLeft_A = digitalRead( E0_A_PIN ); // 26 the same as A8
  ...
  // State :  New A,  New B,  Old A,  Old B.
  byte state = 0;                   
  state = state | ( newLeft_A  << 3 );
  state = state | ( newLeft_B  << 2 );
  state = state | ( oldLeft_A  << 1 );
  state = state | ( oldLeft_B  << 0 );
  ...
  switch( state ) 
  {
    ...
    default:               break;  // No movement/invalid.
  }
